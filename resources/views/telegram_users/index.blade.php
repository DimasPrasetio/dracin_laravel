...
@section('title', 'Bot Users')

@section('content')
<div class="animate-fade-in space-y-6">
    <div class="flex flex-col space-y-4 md:flex-row md:items-center md:justify-between md:space-y-0">
        <div>
            <h2 class="text-3xl font-bold text-gray-900">Bot Users</h2>
            <p class="mt-1 text-sm text-gray-600">Semua bot users berasal dari tabel users (per kategori)</p>
        </div>
    </div>
    ...
    <div class="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
        <div class="px-6 py-5 border-b border-gray-100">
            <div class="flex flex-col space-y-4 md:flex-row md:items-center md:justify-between md:space-y-0">
                <div>
                    <h3 class="text-lg font-semibold text-gray-900">All Bot Users</h3>
                    ...
                    <div class="flex-1 px-6 py-4 overflow-y-auto space-y-4">
                        <div class="rounded-2xl border border-gray-200 bg-white shadow-sm">
                            ...
                            <div class="p-4">
                                ...
                                <p class="mt-2 text-xs text-gray-500">Minimal 1 hari.</p>
                            </div>
                        </div>
                    </div>
...
$(document).ready(function() {
        ...
        users.forEach(function(user) {
            ...
            const row = `
                <tr class="hover:bg-blue-50/50 transition-all duration-200 group">
                    ...
                    <td class="px-6 py-4">
                        <span class="text-sm font-mono text-gray-700">${user.telegram_id}</span>
                    </td>
                    <td class="px-6 py-4">
                        ${vipBadge}
                    </td>
    ...
    $('#vipForm').on('submit', function(e) {
        ...
        $.ajax({
            ...
            data: { vip_days: vipDays, category_id: $('#categoryFilter').val() },
            success: function(response) {
                closeLoading();
                if (response.ok) {
                    closeVipModal();
